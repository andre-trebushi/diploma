...
def plot_dfls(dfls, labels=None, domains='s', scale='mm', title=None,
            x_lim=None, y_lim=None, slice_xy=True, phase=False, savefig=False, showfig=True, filePath=None, fig_name=None):
    ...
    for dfl in dfls:
        ...
        if slice_xy is True:   
            fig.text(0.01, 0.01, 'x- y- срез', fontsize=18)
            I_x = np.sum(dfl.intensity(), axis=0)[dfl.Ny()//2+1, :]
            I_y = np.sum(dfl.intensity(), axis=0)[:, dfl.Nx()//2+1]
        elif slice_xy is False:
            fig.text(0.01, 0.01, 'x- y- проекция', fontsize=18)
            I_x = np.sum(dfl.intensity(), axis=(0,1))
            I_y = np.sum(dfl.intensity(), axis=(0,2))
        else: 
            raise AttributeError('slice_xy is a boolean type')
            _logger.error(ind_str + 'slice_xy is a boolean type')
